...
for(iter=1:Niter)
    ...
    %q2t=u2'*Qtrn; 
    if(varcorr==0)
        q2t = l2(1:kmax,1:kmax)*v2(:,1:kmax)';    
    else
        for(j=1:size(Qtrn,2))
            q2t_del =Qtrn;
            q2t_del(:,j)=[];
    ...
    w0 = pinv( (q2t(1:k,:)*q2t(1:k,:)'))*q2t(1:k,:)*ytrn;
    
    %----temporary
    %errt = ( (q2t(1:k,:)'*w0) - ytrn(:) ).^2;
    %W    = diag(errt);
    %w0   = pinv( (q2t(1:k,:)*W*q2t(1:k,:)'))*q2t(1:k,:)*W*ytrn;
    %----temporary
    
    y_hat = q2h(1:k,:)'*w0;
    PRESS_tot(k) = PRESS_tot(k) + sum( (y_hat - ytst).^2 );
...
if( vxx > 0.05 )
    ...
    for(bsr=1:1000)
        ...
        yt = y(list);
        yt = yt-mean(yt);
        w0 = pinv( (q2t(1:ixx,:)*q2t(1:ixx,:)') )*q2t(1:ixx,:)*yt;
        map(:,bsr) = U*u2(:,1:ixx)*w0;
    end
