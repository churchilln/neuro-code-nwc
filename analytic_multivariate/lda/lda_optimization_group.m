function result_set = lda_optimization_group ( data_class1, data_class2, drf, N_iters )
%
...
%% 2. initial data reduction factor + generate fulldata matrix
    ...
    K_max = min( [ floor((N_class1_full+N_class1_full)/2) round(drfPCs/2)] ) - 1;
...
for( iter = 1:N_iters )
    ...
    list = randperm( Nsubj );
    %----%
    list1= list(                1: round(Nsubj/2));
    list2= list( round(Nsubj/2)+1: end           );
