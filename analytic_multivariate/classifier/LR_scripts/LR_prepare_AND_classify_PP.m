function [ output ] = LR_prepare_AND_classify_PP( data_trn, data_tst, REF_IMG, labels, data_HELD, labels_HELD, Range, RangeVal2, regtype, mask )
%
...
if( strcmp(regtype,'L1') )
    ...
    map_trn   = LR_map (model_trn, data_trn);
    %
    ...
    map_tst   = LR_map (model_tst, data_tst);
    %
    ...
    % classify
    [accur_1] = LR_classify_PP (model_trn, data_trn, trn_avg, data_tst, labels);
    [accur_2] = LR_classify_PP (model_tst, data_tst, tst_avg, data_trn, labels);
    % prediction
    PP = (accur_1 + accur_2)./2;
    
    % classify
    [accur_1] = LR_classify_PP (model_trn, data_trn, trn_avg, data_HELD, labels_HELD);
    [accur_2] = LR_classify_PP (model_tst, data_tst, tst_avg, data_HELD, labels_HELD);
    % prediction
...
elseif( strcmp(regtype,'PC') )
    ...
    map_trn   = LR_map (model_trn, trn_u(:,1:Range));
    %
    CC=corrcoef([REF_IMG ,map_trn]);
    CC=sign(diag( CC(1:Range,Range+1:end) ));
    map_trn=map_trn*diag(CC);
    ...
    map_tst   = LR_map (model_tst, tst_u(:,1:Range));
    %
    CC=corrcoef([REF_IMG ,map_tst]);
    CC=sign(diag( CC(1:Range,Range+1:end) ));
    map_tst=map_tst*diag(CC);
    ...
    % classify 
    [accur_1] = LR_classify_PP (model_trn, trn_u(:,1:Range), trn_avg, data_tst, labels);
    [accur_2] = LR_classify_PP (model_tst, tst_u(:,1:Range), tst_avg, data_trn, labels);
    %
    PP = (accur_1 + accur_2)./2;
    
    % classify 
    [accur_1] = LR_classify_PP (model_trn, trn_u(:,1:Range), trn_avg, data_HELD, labels_HELD);
    [accur_2] = LR_classify_PP (model_tst, tst_u(:,1:Range), tst_avg, data_HELD, labels_HELD);
    %
    GG = (accur_1 + accur_2)./2;
...
elseif(  strcmp(regtype,'IC') )
    ...
    map_trn   = LR_map (model_trn, trn_u_ic(:,1:Range));
    %
    CC=corrcoef([REF_IMG ,map_trn]);
    CC=sign(diag( CC(1:Range,Range+1:end) ));
    map_trn=map_trn*diag(CC);
    ...
    map_tst   = LR_map (model_tst, tst_u_ic(:,1:Range));
    %
    CC=corrcoef([REF_IMG ,map_tst]);
    CC=sign(diag( CC(1:Range,Range+1:end) ));
    map_tst=map_tst*diag(CC);
    ...
    % classify 
    [accur_1] = LR_classify_PP (model_trn, trn_u_ic(:,1:Range), trn_avg, data_tst, labels);
    [accur_2] = LR_classify_PP (model_tst, tst_u_ic(:,1:Range), tst_avg, data_trn, labels);
    %
    PP = (accur_1 + accur_2)./2;
    
    % classify 
    [accur_1] = LR_classify_PP (model_trn, trn_u_ic(:,1:Range), trn_avg, data_HELD, labels_HELD);
    [accur_2] = LR_classify_PP (model_tst, tst_u_ic(:,1:Range), tst_avg, data_HELD, labels_HELD);
    %
    GG = (accur_1 + accur_2)./2;
