function [ EIGS ] = LD_prepare_ref( data_ref, labels, Range, RangeVal2, regtype,mask )
%
...
if( strcmp(regtype,'L1') )
    ...
    % run LD
    model_ref = LD_L1_train( dataCov_ref, labels, Range );
...
elseif( strcmp(regtype,'L2') )
    ...
    % run LD
    model_ref = LD_L2_train( dataCov_ref, labels, Range );
    map_ref   = LD_map (model_ref, data_ref);
    % flip eigens. to match the CV scores:
    CVscores = data_ref' * map_ref; 
    sggn = sign( mean(CVscores( labels==1,: )) - mean(CVscores( labels==0,: )) ); % 
...
elseif( strcmp(regtype,'PC') )
    ...
    [v, s] = svd (dataCov_ref); s=sqrt(s); ref_u=data_ref*v*inv(s);
     ref_Z = s*v';
    % run LD
    model_ref = LD_NR_train (ref_Z, labels, Range );
    map_ref   = LD_map (model_ref, ref_u(:,1:Range));
    % flip eigens. to match the CV scores:
...
elseif(  strcmp(regtype,'IC') )
    ...
    model_ref = LD_NR_train (ref_S, labels, Range);
    map_ref   = LD_map (model_ref, ref_u_ic(:,1:Range));
