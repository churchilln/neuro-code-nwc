function allright = LR_IC_run2 (cond1, cond2, subBLK)
% clear;
% close all;
% cond1=3;
% cond2=5;
...
for (subj_idx=1:18)
 ...
 for(maxPCs = [subBLK-2, 2*subBLK-2] )
...
%% LD-PC, 4x20 blocks (LVL 111)
    ...
    tst_S = tst_u_ic'*tst_data;
            ...
            model_trn = LR_PC_create (trn_S, Label_splt, maxPCs);
            map_trn   = LR_map (model_trn, trn_u_ic (:, 1:maxPCs));
            %
            CC=corrcoef([map_ref ,map_trn]);
            CC=sign(diag( CC(1:maxPCs,maxPCs+1:end) ));
            map_trn=map_trn*diag(CC);

            model_tst = LR_PC_create (tst_S, Label_splt, maxPCs);
            map_tst   = LR_map (model_tst, tst_u_ic (:, 1:maxPCs));
            %
        ...
        [accur_1] = LR_classify_set (model_trn, trn_u_ic (:, 1:maxPCs), trn_avg, tst_data, Label_splt);
        % classify training data using test model
        [accur_2] = LR_classify_set (model_tst, tst_u_ic (:, 1:maxPCs), tst_avg, trn_data, Label_splt);
            ...
            %% %% %% -------- %% %% %% -------- %% %% %%
                ...
                % classify test data using training model
                [genr_1] = LR_classify_set (model_trn, trn_u_ic (:, 1:maxPCs), trn_avg, held_data, Label_held);
                % classify training data using test model
                [genr_2] = LR_classify_set (model_tst, tst_u_ic (:, 1:maxPCs), tst_avg, held_data, Label_held);
        ...
        save(strcat('LR_IC_sub',num2str(subBLK),'alt_C',num2str(10*cond1+cond2),'_maxPCs',num2str(maxPCs),'_subj_',num2str(subj_idx),'_',num2str(split),'.mat'),...
