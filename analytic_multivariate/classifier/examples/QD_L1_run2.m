function allright = QD_L1_run2 (cond1, cond2, subBLK)
% clear;
% close all;
% cond1=3;
% cond2=5;
...
for (subj_idx=5:18)
    ...
    model_all = QD_L1_create (all_K, Label_full,subBLK*2-2, Range);
    map_ref   = QD_map_signed_set (model_all, full_data, all_K, 'L1' );
...
%% LD-PC, 4x20 blocks (LVL 111)
    ...
    for split = 1:n_splits
        ...
        for(swap=1:2)
            ...
            model_trn = QD_L1_create (trn_K, Label_splt,2*subBLK-2, Range);
            map_trn   = QD_map_signed_set (model_trn, trn_data, trn_K, 'L1' );
            %
            CC=corrcoef([map_ref ,map_trn]);
            CC=sign(diag( CC(1:Range,Range+1:end) ));
            map_trn=map_trn*diag(CC);

            model_tst = QD_L1_create (tst_K, Label_splt,2*subBLK-2, Range);
            map_tst   = QD_map_signed_set (model_tst, tst_data, tst_K, 'L1' );
            %
            ...
            [accur_1] = QD_classify_set (model_trn, trn_data, trn_avg, tst_data, Label_splt,'L1');        
            % classify training data using test model
            [accur_2] = QD_classify_set (model_tst, tst_data, tst_avg, trn_data, Label_splt,'L1');        
            ...
            %% %% %% -------- %% %% %% -------- %% %% %%
                ...
                % classify test data using training model
                [genr_1] = QD_classify_set (model_trn, trn_data, trn_avg, held_data, Label_held,'L1');        
                % classify training data using test model
                [genr_2] = QD_classify_set (model_tst, tst_data, tst_avg, held_data, Label_held,'L1');        
        ...
        %outputs/QD_sub/
        save(strcat('QD_L1_sub',num2str(subBLK),'alt_C',num2str(10*cond1+cond2),'_subj_',num2str(subj_idx),'_',num2str(split),'.mat'),...
