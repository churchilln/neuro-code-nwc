function allright = LR_L2_run2 (cond1, cond2, subBLK)
% clear;
% close all;
% cond1=3;
% cond2=5;
...
for (subj_idx=1:18)
    ...
    model_all = LR_L12_create (all_K, Label_full,'L2', Range);
    map_ref   = LR_map (model_all, full_data);
...
%% LD-PC, 4x20 blocks (LVL 111)
    ...
    for split = 1:n_splits
        ...
        for(swap=1:2)
            ...
            model_trn = LR_L12_create (trn_K, Label_splt,'L2', Range);
            map_trn   = LR_map (model_trn, trn_data);
            %
            CC=corrcoef([map_ref ,map_trn]);
            CC=sign(diag( CC(1:Range,Range+1:end) ));
            map_trn=map_trn*diag(CC);

            model_tst = LR_L12_create (tst_K, Label_splt,'L2', Range);
            map_tst   = LR_map (model_tst, tst_data);
            %
            ...
            [accur_1] = LR_classify_set (model_trn, trn_data, trn_avg, tst_data, Label_splt);
            % classify training data using test model
            [accur_2] = LR_classify_set (model_tst, tst_data, tst_avg, trn_data, Label_splt);
            ...
            %% %% %% -------- %% %% %% -------- %% %% %%
                ...
                % classify test data using training model
                [genr_1] = LR_classify_set (model_trn, trn_data, trn_avg, held_data, Label_held);
                % classify training data using test model
                [genr_2] = LR_classify_set (model_tst, tst_data, tst_avg, held_data, Label_held);
        ...
        save(strcat('LR_L2_sub',num2str(subBLK),'alt_C',num2str(10*cond1+cond2),'_subj_',num2str(subj_idx),'_',num2str(split),'.mat'),...
