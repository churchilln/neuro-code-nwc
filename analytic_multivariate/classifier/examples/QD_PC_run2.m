function allright = QD_PC_run2 (cond1, cond2, subBLK)
% clear;
% close all;
% cond1=3; cond2=5; subBLK=5;
...
for (subj_idx=1:18)
    ...
    model_all = QD_PC_create (all_Z, Label_full, maxPCs);
    map_ref   = QD_map_signed_set (model_all, all_u(:,1:maxPCs), all_Z(1:maxPCs,:),'PC' );
...
%% LD-PC, 4x20 blocks (LVL 111)
    ...
    for split = 1:n_splits
        ...
        for(swap=1:2)
            ...
            model_trn = QD_PC_create (trn_Z, Label_splt, maxPCs);
            map_trn   = QD_map_signed_set (model_trn, trn_u(:,1:maxPCs), trn_Z(1:maxPCs,:),'PC' );
            %
            CC=corrcoef([map_ref ,map_trn]);
            CC=sign(diag( CC(1:maxPCs,maxPCs+1:end) ));
            map_trn=map_trn*diag(CC);

            model_tst = QD_PC_create (tst_Z, Label_splt, maxPCs);
            map_tst   = QD_map_signed_set (model_tst, tst_u(:,1:maxPCs), tst_Z(1:maxPCs,:),'PC' );
            %
            ...
            [accur_1] = QD_classify_set (model_trn, trn_u(:,1:maxPCs), trn_avg, tst_data, Label_splt,'PC');        
            % classify training data using test model
            [accur_2] = QD_classify_set (model_tst, tst_u(:,1:maxPCs), tst_avg, trn_data, Label_splt,'PC');        
            ...
            % classify test data using training model
            [genr_1] = QD_classify_set (model_trn, trn_u(:,1:maxPCs), trn_avg, held_data, Label_held,'PC');        
            % classify training data using test model
            [genr_2] = QD_classify_set (model_tst, tst_u(:,1:maxPCs), tst_avg, held_data, Label_held,'PC');        
        ...
        save(strcat('QD_PC_sub',num2str(subBLK),'alt_C',num2str(10*cond1+cond2),'_subj_',num2str(subj_idx),'_',num2str(split),'.mat'),...
